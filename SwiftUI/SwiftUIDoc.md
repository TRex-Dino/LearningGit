...
# 📘 Полная теория: `Alert` в SwiftUI

---

### 🔹 Что такое `Alert`

> `Alert` — это модальное окно с сообщением и кнопками. Оно **не блокирует интерфейс**, но требует от пользователя внимания и ответа.
...
.alert(isPresented: $showAlert) {
    Alert(title: Text("Заголовок"))
}
```

* `isPresented:` — биндинг на `Bool`, когда `true` — алерт появляется
* Внутри ты создаёшь `Alert(...)` с кнопками, заголовком и сообщением

---

### 🔸 Вариант 1: Один заголовок
...
> Синтаксис iOS 15+ (с короткой формой)

---

### 🔸 Вариант 2: Заголовок + сообщение + действия
...
* `primaryButton` и `secondaryButton` задают порядок и стиль
* `.cancel()` автоматически получает стиль "Отмена"
...
* `Alert` поддерживает **только 1 или 2 кнопки**
* Если тебе нужно больше опций — используй `confirmationDialog`
...
| `role:`         | Стиль                 |
| --------------- | --------------------- |
| `.cancel`       | Серая, всегда внизу   |
| `.destructive`  | Красная               |
| `nil` (default) | Синяя (или системная) |

---

## 🔸 Пример: алерт с удалением

```swift
@State private var showAlert = false
@State private var message = "Ничего не удалено"
...
| ------------------ | ------------------------ | -------------------------------------- |
| Кол-во кнопок      | 1–2                      | Сколько хочешь                         |
| Расположение       | По центру                | Внизу                                  |
| Тип взаимодействия | Быстрое подтверждение    | Меню выбора (например, фото, действия) |
| Использовать для   | Удаления, предупреждений | Меню выбора, параметров                |

---

## ✅ Когда использовать:

* `Alert` — если нужно **внимание + подтверждение**
* `ConfirmationDialog` — если нужен **выбор из нескольких опций**
* `Sheet` — для сложных UI и кастомного контента
...
| Проблема                        | Причина                                                           |
| ------------------------------- | ----------------------------------------------------------------- |
| Алерт не появляется             | `$showAlert` не стал `true` или тело алерта не создаётся          |
| Алерт мгновенно исчезает        | Состояние сбрасывается до отрисовки (например, `@State` сброшено) |
| Алерт с 3+ кнопками не работает | SwiftUI `Alert` не поддерживает больше 2 кнопок                   |

---

# 📘 Полная теория: `confirmationDialog` в SwiftUI

---

## 🔹 Что такое `confirmationDialog`

> `confirmationDialog` — это модальное окно, **появляющееся снизу**, которое показывает **список действий**.
> По внешнему виду оно похоже на `ActionSheet` в старом UIKit.

✅ Подходит для: выбора вариантов, контекстных действий, подтверждения перед выполнением
✅ Работает с **несколькими кнопками**
✅ Поддерживает `.destructive`, `.cancel`, `.default` роли
...
| `isPresented`            | Управляет показом                 |
| `titleVisibility`        | `.visible` или `.hidden`          |
| `actions` (внутри body)  | Кнопки, меню, роли                |
| `message:` (опционально) | Дополнительное описание (iOS 16+) |
...
|              | `Alert`                   | `ConfirmationDialog`  |
| ------------ | ------------------------- | --------------------- |
| Кнопок       | Макс. 2                   | Сколько угодно        |
| Расположение | Центр                     | Низ экрана            |
| Подходит для | Критических подтверждений | Меню, выбор, действия |
| Отмена       | `.cancel()`               | `role: .cancel`       |

---

## 🔹 Поддержка `role` в кнопках

* `.cancel` — системная кнопка "Отмена"
* `.destructive` — красный цвет, для опасных действий
* `.none` — обычная системная кнопка
...
struct ConfirmationExample: View {
    @State private var showDialog = false
    @State private var selectedAction: String = "Ничего не выбрано"

    var body: some View {
        VStack(spacing: 20) {
            Text("Действие: \(selectedAction)")
        ...
        .confirmationDialog("Выберите действие", isPresented: $showDialog, titleVisibility: .visible) {
            Button("Скопировать") { selectedAction = "Скопировано" }
            Button("Удалить", role: .destructive) { selectedAction = "Удалено" }
            Button("Отмена", role: .cancel) { selectedAction = "Отменено" }
        }
    }
}
```
...
| Поведение                 | Причина                                          |
| ------------------------- | ------------------------------------------------ |
| Диалог не появляется      | `$showDialog` не становится `true`               |
| Диалог сразу закрывается  | Стейт сбрасывается или ошибка в логике действий  |
| Не отображается заголовок | `titleVisibility: .hidden` по умолчанию в iOS 15 |

---

## 🎯 Best practices

* Используй `.alert` — для критических предупреждений
* Используй `.confirmationDialog` — когда у пользователя **несколько вариантов выбора**
* Не перегружай диалог: **3–5 опций максимум**
* Используй `@State` для отслеживания результата выбора

---

## ✅ Когда особенно полезно:

* Выбор источника загрузки (Камера / Галерея)
* Действия над элементом (Редактировать / Удалить / Скопировать)
* Переключение между фильтрами / режимами
* Быстрая смена темы или языка

---

# 📘 Полная теория: `contextMenu` в SwiftUI

---

## 🔹 Что такое `contextMenu`

> `contextMenu` — это выпадающее меню, которое появляется при **долгом нажатии** (iOS) или **правом клике** (macOS/iPadOS) на элементе.
> Оно позволяет показать **набор второстепенных действий**, не загромождая интерфейс.
...
* У элементов списка (`List`)
* У иконок, изображений, ячеек
* Для показа **альтернативных/дополнительных действий**
* Как замена или дополнение к `swipeActions`, `longPressGesture`, `toolbar`, `confirmationDialog`

---

## 🔹 Возможности `contextMenu`

| Возможность             | Пример                             |
| ----------------------- | ---------------------------------- |
| Несколько кнопок        | Через `Button` в `contextMenu`     |
| Стиль кнопок            | `role: .destructive`, `.cancel`    |
| Иконки                  | `Label("...", systemImage: "...")` |
| Видимость по условию    | Через `if` внутри меню             |
| Адаптация под платформу | Работает на iOS, iPadOS, macOS     |
    ...
    ForEach(fruits, id: \.self) { fruit in
        Text(fruit)
            .contextMenu {
                ...
                Button("Копировать") {
                    UIPasteboard.general.string = fruit
                }
            }
    }
}
...
## 📌 Когда использовать `contextMenu`

| Использовать если…                     | Вместо чего                        |
| -------------------------------------- | ---------------------------------- |
| Хочешь скрыть второстепенные действия  | Кнопки прямо в UI                  |
| Нужно показать только по нажатию/клику | `swipeActions`, `toolbar`, `sheet` |
| Хочешь показать действия по элементу   | `Alert`, `confirmationDialog`      |
...
| -------------------------------- | ----------------------------------- |
| Нет вложенных меню               | Только плоский список действий      |
| Не видно, пока не нажмёшь        | Неинтуитивно на iOS (без подсказки) |
| Не поддерживает `.disabled(...)` | Ограничено по условной доступности  |

---

## 🎯 Best Practices

* Используй `Label` с `systemImage` — так понятнее действия
* Размещай `contextMenu` **на интерактивных элементах** (текст, иконки, карточки)
* Если действий слишком много — **лучше использовать `confirmationDialog`**
